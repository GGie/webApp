		        ...
		        foreach($queries->rajaongkir->results as $results ){ 
		          foreach($results->costs as $costs) {
		          	    ...
		          	    $return .= '<label class="c-input-label">';
					        $return .= '<input type="radio" class="co-input-radio" value="jnt_reg" name="kurir" onclick="selectKurir(\'' . $costs->cost[0]->value . '\')">';
					        $return .= '<span class="co-input__inner-label o-flag">';
					            ...
					            $return .= '<div class="o-flag__body">';
					                ...
					                $return .= '<div class="u-fg--ash-dark"><span>' . number_format($cost->value, 0,',', '.') . '</span> (' . $cost->etd . ' hari kerja)</div>';
					            	}
					            $return .= '</div>';
					        $return .= '</span>';
					    $return .= '</label>';
